...
select gender, ROUND(SUM(purchase_amount),0) as revenue
from customer
group by gender

--Q2.Which customers used discount but still spent more than the average purchase amount?
select customer_id, ROUND(purchase_amount,0) as purchase_amount
from customer
...
--Q9 Are customers who are repeat buyers (more than 5 previous purhcases) also likely to subscribe?
